...
import com.accenture.interviewproj.utils.JobUtility;
...
public class JobRestController {
	...
	public ResponseEntity<List<?>> getAllJobs() {
		return ResponseEntity.ok(jobService.displayListOfJobs());
	}

	@GetMapping("/{jobName}")
	public ResponseEntity<?> get(@PathVariable("jobName") String jobName) {
		Job job = jobService.searchByJobName(jobName);
		if (job != null) {
	...
	public ResponseEntity<?> updateIncident(@RequestBody Job job) {
		Job updateJob = null;
		try {
			updateJob = jobService.updateJob(job);
		} catch (JobNotFoundException e) {
			e.printStackTrace();
		}
		if (updateJob == null) {
