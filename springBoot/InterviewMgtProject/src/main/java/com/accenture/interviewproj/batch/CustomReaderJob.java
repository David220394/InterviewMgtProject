 ...
 */
public class CustomReaderJob implements ItemReader<Row> {
	
	private ItemReader<Row> delegate;
	
	String path;
	
	public CustomReaderJob(String path){
		this.path = path;
	}
	...
	public Row read()
			throws Exception {
		if(delegate == null) {
			delegate = new IteratorItemReader<>(job());
		}
	 ...
	 */
	private List<Row> job() throws EncryptedDocumentException, 
