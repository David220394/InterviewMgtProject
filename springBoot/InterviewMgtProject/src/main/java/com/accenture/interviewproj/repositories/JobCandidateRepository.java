...
public interface JobCandidateRepository extends JpaRepository<JobCandidate, Long> {
	...
	@Query(value="SELECT s.STATUS_NAME FROM TABLE_STATUS s INNER JOIN TABLE_JOB_CANDIDATE jc ON jc.JOB_CANDIDATE_ID = s.CANDIDATE_ID WHERE jc.CANDIDATE_ID=? AND jc.JOB_ID=? ORDER BY s.status_change_date DESC", nativeQuery=true)
